...
class class_db {
    private $_db;       // variable contenant l'instance de connection à la bd
    private $_user;    // table série en fonction de la restriction du l'utilisateur
    private $_serie;    // table série en fonction de la restriction du l'utilisateur
    
    public function __construct($pageRestriction) {
        require_once 'connectBDD.php';
        $this->_db = Db::getInstance();         // création de l'intance de connection à la bd
        if(isset($_COOKIE['log'])){
            $this->_user = $_COOKIE['log'];
            setcookie('log', $this->_user , time() + 365*24*3600, null, null, false, true);
        ...
        if($this->restriction() == 1){
            $this->_serie = 'serietp';
        }elseif($this->restriction() == 0){
            $this->_serie = 'serie';
        }elseif($this->restriction() == NULL && $pageRestriction == 'false'){
